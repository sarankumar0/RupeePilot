...
export default function ExpenseSummary({ summary, expenses, monthlyBudget, monthlyIncome }: Props) {
  ...
  const [periodA, setPeriodA] = useState(() => {
    const d = new Date(); d.setDate(d.getDate() - 7); return toISOWeek(d);
  });
  ...
  const toggleInactive = 'text-gray-400 hover:bg-gray-800';
    ...
    <div className="space-y-6">
      ...
      {categories.length > 0 && (
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className={`px-3 py-1 rounded-full text-sm font-medium transition ${!activeCategory ? 'bg-white text-gray-900' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}
          >
            All
          </button>
          {categories.map(cat => (
            <button
              ...
              style={{
                backgroundColor: activeCategory === cat ? (CATEGORY_COLORS[cat] ?? '#6b7280') : '#1f2937',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-gray-900 rounded-xl p-6">
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            <div className="ml-auto text-right">
              <p className="text-gray-500 text-xs">{mainChartTitle}</p>
              <p className="text-white font-bold text-lg">{fmt(mainChartTotal)}</p>
            </div>
          </div>
              ...
              <BarChart data={mainChartData} barSize={chartPeriod === 'month' ? 10 : 30}>
                ...
                <Bar dataKey="total" radius={[5, 5, 0, 0]}>
                  {mainChartData.map((entry, i) => (
                    <Cell key={i}
                      fill={chartPeriod === 'day' ? (CATEGORY_COLORS[entry.name] ?? '#6b7280') : '#3b82f6'}
                    />
                  ))}
                </Bar>
              </BarChart>
        ...
        <div className="space-y-4">
          {/* 30-day trend */}
          <div className="bg-gray-900 rounded-xl p-5">
            <p className="text-gray-400 text-sm font-medium mb-3">30-Day Trend</p>
          ...
          {(weekendAvg > 0 || weekdayAvg > 0) && (
            <div className="bg-gray-900 rounded-xl p-5">
              <p className="text-gray-400 text-sm font-medium mb-3">Spend Pattern</p>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-400">Weekday avg</span>
                  <span className="text-white font-semibold">{fmt(weekdayAvg)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Weekend avg</span>
                  <span className={`font-semibold ${weekendAvg > weekdayAvg ? 'text-yellow-400' : 'text-green-400'}`}>
                    {fmt(weekendAvg)}
                  </span>
                </div>
                {weekendAvg > weekdayAvg && weekdayAvg > 0 && (
                  <p className="text-yellow-500 text-xs pt-1">
                    ⚠️ {Math.round(((weekendAvg - weekdayAvg) / weekdayAvg) * 100)}% more on weekends
                  </p>
                )}
                {heaviestDay && (
                  <p className="text-gray-500 text-xs">📆 Heaviest: {heaviestDay}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="bg-gray-900 rounded-xl p-5">
            ...
            <p className="text-2xl font-bold text-white">{fmt(summary.allTimeTotal)}</p>
            <p className="text-gray-500 text-xs mt-1">{summary.totalCount} expenses</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 rounded-xl p-6">
        <h3 className="text-gray-300 font-semibold mb-4">Compare Periods</h3>
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <div className="flex rounded-lg overflow-hidden border border-gray-700">
            {(['day', 'week', 'month'] as const).map(p => (
              <button key={p}
                onClick={() => { setCompareType(p); resetComparePeriods(p); }}
                className={`${toggleBase} ${compareType === p ? 'bg-purple-600 text-white' : toggleInactive}`}>
                {p}
              </button>
            ))}
          </div>
      ...
      <div className="bg-gray-900 rounded-xl p-6">
        <div className="flex flex-wrap items-center gap-3 mb-5">
          <h3 className="text-gray-300 font-semibold">Expenses</h3>
          ...
          <div className="ml-auto flex items-center gap-4">
            <span className="text-gray-500 text-sm">{recentExpenses.length} items</span>
            {recentDayTotal > 0 && (
              <span className="text-white font-semibold">{fmt(recentDayTotal)}</span>
            )}
          </div>
        </div>
          ...
          <ul className="divide-y divide-gray-800">
            {recentExpenses.map(e => (
              <li key={e._id} className="flex items-center justify-between py-3">
                ...
                <p className="text-white font-semibold">{fmt(e.amount)}</p>
              </li>
            ))}
          </ul>
        )}
